...
Un par spec+tasks por plan, vinculados por nombre:

```
sdlc/
├── specs/
│   └── 2026-08-22-user-auth-refactor.md      # el plan revisable
└── tasks/
    └── 2026-08-22-user-auth-refactor.md      # checklist secuencial de tareas
```

- Mismo slug y fecha en ambos = vínculo implícito por convención.
...
- El archivo de tareas usa checklist (`- [ ]` / `- [x]`) con IDs cortos por tarea (T1, T2...). Ese formato es lo que permite retomar tras un crasheo: el estado queda persistido en el repo vía los propios commits, no en la memoria del agente.
- Cada commit referencia su tarea (`T3: ...`) para trazabilidad bidireccional commit ↔ checklist.

## Flujo completo

```
Issue + label "sdlc-plan"
   └─> [WF1 · Planner] agente investiga repo → genera AMBOS archivos en UN solo PR
        "[spec] 2026-08-22-slug" vinculado al issue
             ├─ Comentás correcciones → agente re-procesa y actualiza el PR
             └─ Aprobación + merge a main
                  └─> [WF2 · Dispatcher] push a main con paths sdlc/specs/**
...
- **Trigger**: únicamente label específico sobre issues (ej. `sdlc-plan`). Nada de disparar en toda apertura de issue.
...
- **Iteración**: el workflow también escucha comentarios en ese PR para que el agente incorpore tus correcciones editando la misma rama.
- **Safe outputs**: `create-pull-request` únicamente; permisos read-only salvo esa salida.

## WF2 · Dispatcher (workflow liviano, casi sin IA)

- **Trigger**: `push` a main filtrando `sdlc/specs/**`. Determinístico: el merge ES la señal.
...
- Trabaja en rama dedicada (`sdlc/<date>-<slug>`), ejecuta las tareas del checklist en orden estricto, un commit atómico por tarea, actualizando el checkbox en el propio commit.
